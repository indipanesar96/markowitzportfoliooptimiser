...
def plot_ef():
    ...
    plt.errorbar(timings['OOS Var'], timings['OOS Return'],
                 yerr=timings['OOS Var'] / len(timings['OOS Var']), label=r"Out Of Sample")
    ...
    plt.errorbar(timings['IS Var'], timings['IS Return'],
                 yerr=timings['IS Var'] / len(timings['IS Var']), label=r"In Sample")
    plt.xlabel(r"Portfolio Variance")
    plt.ylabel(r"Portfolio Returns")
    # plt.legend(loc=r'best')
    plt.title(r"Efficient Frontier")
...
def plot_linear():
    ...
    plt.errorbar(timings['Target Return'], timings['OOS Return'],
                 yerr=timings['OOS Var'] / len(timings['OOS Var']), ecolor='g', label=r"Out Of Sample")

    plt.errorbar(timings['Target Return'], timings['IS Return'],
                 yerr=timings['IS Var'] / len(timings['IS Var']), c="orange", ecolor='g', linestyle="--", label=r"In Sample")
...
def plot_order():
    ...
    ax0.plot(assets_variation['nAssets'], assets_variation[f'Fitted: 2'], linestyle="--",
             label=f'(Order, MSE rank): (2, 1)')
    ax0.plot(assets_variation['nAssets'], assets_variation[f'Fitted: 3'], label=f'(Order, MSE rank): (3, 2)')
...
def plot_shorts():
    ...
    plt.plot(nShorts['rp'], nShorts['aveShorts'])
